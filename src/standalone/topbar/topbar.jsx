...
export default class Topbar extends React.Component {
  ...
  saveAsJson = () => {
    ...
    if (this.hasLevelError()) {
      localStorage.setItem('prettyJsonContent', 'hasErr')
    } else {
      ...
      localStorage.setItem('prettyJsonContent', prettyJsonContent)
    }
  }
  ...
  // 有报错情况
  hasLevelError = () => {
    return this.props.errSelectors.allErrors().filter(err => err.get("level") === "error").size > 0
  }
    ...
    return (
      <div className="swagger-editor-standalone">
        <div className="topbar">
          <div className="topbar-wrapper" style={{ display: 'none' }}>
            <Link href="#">
          ...
          <div className="topbar-wrapper" style={{ display: 'none' }}>
            <span className="save-as-json" onClick={this.saveAsJson}>保存JSON文件</span>
          </div>
        </div>
      </div>
    )
